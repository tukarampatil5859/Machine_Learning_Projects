...
def ocr_core(filename):
   ...
   # cv2.imshow('testing',img)
    ...
    # Pytesseract uses OCR here to recognise these charaters
        ...
        print("--------------------------------------------------------------------------")
        print("IMAGE TO NUMBER PLATE TEXT IS :")
        print(text)
        print("--------------------------------------------------------------------------")
        mycursor = mydb.cursor()
        clearText = re.sub(r"[^a-zA-Z0-9]", "", text)
        print("--------------------------------------------------------------------------")
        print("IMAGE TO NUMBER PLATE TEXT AFTER CLEANING IS :")
        print(clearText)
        print("--------------------------------------------------------------------------")
        mycursor.execute("select * from car_info where number_plate=%s",(clearText,))
        result = mycursor.fetchall()
        # result1= re.sub(r"[('datetime.date')]", result)
        print("--------------------------------------------------------------------------")
        ...
        mycursor.close()
        # db = Database()
        # db.select("select * from people where number_plate=%s", (clearText,))
        return clearText
